...
public:
        ...
        : mRank(static_cast<int>(rank)), mOutputDtype(outputDtype)
    {
        ...
        mReusableTensor = torch::from_blob(
        ...
        at::Tensor const& globalScale)
    {
        ...
        TORCH_CHECK(mat1.data_ptr() != nullptr, "mat1.data_ptr() is nullptr");
        TORCH_CHECK(mat2.data_ptr() != nullptr, "mat2.data_ptr() is nullptr");
        TORCH_CHECK(mat1Scale.data_ptr() != nullptr, "mat1Scale.data_ptr() is nullptr");
        TORCH_CHECK(mat2Scale.data_ptr() != nullptr, "mat2Scale.data_ptr() is nullptr");
        TORCH_CHECK(globalScale.data_ptr() != nullptr, "globalScale.data_ptr() is nullptr");
        TORCH_CHECK(mWorkspace.get() != nullptr, "mWorkspace is nullptr");
        ...
        printf("Launching GEMM+AR with launch config: %s\n", mGemm->getSupportedLaunchConfigs()[0].str().c_str());

        GemmAllReduceImplInterface::ProblemArgs args;
        args.argProblemShape(m, n, k, 1)
            .argA(mat1.data_ptr())
            .argB(mat2.data_ptr())
            .argD(mOutput.getUnicastPointer(), mOutput.getMulticastPointer(), (void**)mOutput.getIpcUnicastPointers())
            .argAScale(mat1Scale.data_ptr())
            .argBScale(mat2Scale.data_ptr())
            .argBeta(0.f) // no bias
            // .argAlphaPtr((float const*)(globalScale.data_ptr()))
            .argRanks(mRank, mTPGroup)
            .argLaunchConfig(mGemm->getSupportedLaunchConfigs()[0])
...
//        cudaStreamSynchronize(stream);
        ...
        at::Tensor output = mReusableTensor.narrow(0, 0, m).narrow(1, 0, n);
